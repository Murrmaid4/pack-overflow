...
class QuestionServiceTest {
    ...
    public class CreateTests {
        @Test
        void shouldCreate() {
            Question beforeAdd = new Question(TestHelper.existingUser, "test add title", "test add body");
            Question expectedAfterAdd = new Question(TestHelper.existingUser, "test add title", "test add body");
            expectedAfterAdd.setQuestionId(3);
            expectedAfterAdd.setCreated(LocalDateTime.now());
            expectedAfterAdd.setUpdated(LocalDateTime.now());
            when(repository.create(beforeAdd)).thenReturn(expectedAfterAdd);

            Question actual = repository.create(beforeAdd);

            assertEquals(expectedAfterAdd, actual);
        }
    }
